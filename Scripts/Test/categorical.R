...
# Given a dataset consisting of  different realizations of a random vector, 
# count the number of observations that are equal to a certain configuration.

# data --> dataframe of categorical variables;
# variables.name --> vector of strings corresponding to the names of the variables to consider;
# variables.values --> configuration to consider.
...
# Compute the contingency table of the observations in data.
...
# Compute the marginal likelihood restricted to a specific subset of nodes of the graph.

# a --> real positive number such that the parameters of the Dirichlet distribution are given by 
#       a/dim(X_s), where X_s is the cartesian space of all the modalities of the variables 
#       in the subset.
...
# Compute the (total) marginal likelihood via the factorization property of decomposable graphs.
...
logMarginalLikelihood = function(adjacencyMatrix, data, a){
  ...
  # We compute cliques and separators of the graph.
  
  decomposition = getCliquesAndSeparators(adjacencyMatrix)
  cliques = decomposition[[1]]
  separators = decomposition[[2]]
  
  # For each clique we compute the associated marginal likelihood and store it in the vector mlC.
  ...
  # For each separator we compute the associated marginal likelihood and store it in the vector mlS.
...
# Compute the (total) marginal likelihood starting from the clique-separator decomposition of the graph.
...
# Metropolis-Hastings algorithm to make inference on the graph generating the data collected in the dataframe "data". 

# n.iter --> length of the chain;
...
# Compute the ratio of the marginal likelihoods restricted to the variables in subset
...
# Compute the (log) predictive distribution of unit in position given by iMask belonging to the cluster
...
DPMixture = function(data, n.iter, burnin, a_alpha, b_alpha, a_pi, b_pi, a = 1){
  ...
  Xi_chain = matrix(NA, n, n.iter) # n x n.iter matrix which collects the cluster indicators of each unit for every iteration 
  
  A_chain = vector(mode = "list", length = n.iter) # List which collect the adjacency matrices of the visited graphs. 
                                                   # In particular, each element of the list is an array of K(t) adjacency matrices,
                                                   # where K(t) is the (random) number of mixture components at iteration t.
  
  alpha_0_chain = rep(NA, n.iter) # Vector which collects the values of the precision parameter (alpha_0) for every iteration
...
DPMixture_Efficient = function(data, n.iter, burnin, a_alpha, b_alpha, a_pi, b_pi, a = 1){
  ...
  A_chain = vector(mode = "list", length = n.iter) # List which collect the adjacency matrices of the visited graphs. In particular, each element of the
